...
public class Game {
	...
	public Game(Player attacker, Player defender){
		...
		middle = 4;
		playingPlayerEnum = PlayerEnum.ATTACKER;
		move = new PawnTaker(this);
		init_game(9,9);
		plays = new Plays(this);
	}
	...
	public boolean canPlay(int l, int c) {
		if(playingPlayerEnum == PlayerEnum.ATTACKER) {
			return getCellContent(l, c) == CellContent.ATTACK_TOWER;
		}
		
		return getCellContent(l, c) == CellContent.DEFENSE_TOWER || getCellContent(l, c) == CellContent.KING;
	}
	...
	public Player getPlayingPlayer() {
		return getPlayingPlayerEnum() == PlayerEnum.ATTACKER ? attacker : defender;
	}
	...
	public boolean undo_move() {
		Play play = getPlays().undo_move();
		if(play != null) {
			for(Map.Entry<Couple<Integer, Integer>, CellContent> entry : play.getModifiedOldCellContents().entrySet()) {
				setContent(entry.getValue(), entry.getKey().getFirst(), entry.getKey().getSecond());
			}

			playingPlayerEnum = getPlayingPlayerEnum().getOpponent();
			return true;
		}
